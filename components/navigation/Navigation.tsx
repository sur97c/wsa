                ...
                <div className="hidden md:flex space-x-4">
                    {filteredRoles.map((role) => (
                        <Link
                            ...
                            className={`text-dark text-center sm:w-[7.5rem] hover:text-primary-hover ${pathname === (role.key === 'home' ? `/${language}` : `/${language}/${role.key}`)
                ...
                <div className="md:hidden mt-2">
                    {filteredRoles.map((role) => (
                        <Link
                            key={role.key}
                            href={role.key === 'home' ? `/${language}` : `/${language}/${role.key}`}
                            className={`block py-2 text-dark hover:text-primary-hover ${pathname === (role.key === 'home' ? `/${language}` : `/${language}/${role.key}`)
                                ? 'text-primary font-extrabold'
                                : ''
                                }`}
